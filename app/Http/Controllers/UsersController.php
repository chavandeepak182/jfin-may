    ...
    public function mypersonal(Request $request)
{
    ...
    if (!$profile) {
        return redirect()->route('error.page')->withErrors('Profile not found.');
    }

    // Fetch notifications with a custom message
    ...
    return view('frontend.profile.personal-info', [
        'user' => $user,
        'profile' => $profile,
        'notifications' => $notifications
    ]);
}
    public function myprofessional(Request $request)
    {
    $section = $request->query('section', 'personal');
    $userId = session('user_id'); // Retrieve user ID from session
    ...
    $user = DB::table('users')->where('id', $userId)->first();
    $professionalDetails = DB::table('professional_details')->where('user_id', $userId)->first();
    return view('frontend.profile.professional-info', compact('user', 'professionalDetails'));
    }
    ...
    public function mydocuments(Request $request)
    {
        ...
        $user = DB::table('users')->where('id', $userId)->first();
        $documents = DB::table('documents')->where('user_id', $userId)->get();
        return view('frontend.profile.document-info', compact('user', 'documents'));
    }
