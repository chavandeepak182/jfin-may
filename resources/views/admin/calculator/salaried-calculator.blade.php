    ...
    $('#eligibilityForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var taxAmount = Math.round(response.taxAmount);
                    var proposedEmi = Math.round(response.proposedEmi);
                    var remainingIncomeAfterTax = Math.round(response.remainingIncomeAfterTax);
                $('#totalIncome').val(response.totalIncome); // Use .val() for input fields
                $('#taxAmount').val(taxAmount);
                $('#remainingIncomeAfterTax').val(remainingIncomeAfterTax);
                $('#proposedEmi').val(proposedEmi);
...
<script>
    ...
    function calculateTotalIncome() {
        ...
        if (monthsSelected > 0) {
            const avgSalary = (totalSalary / monthsSelected).toFixed(0);
...
function calculateCoApplicantTotalIncome() {
    ...
    if (monthsSelected > 0) {
        const avgSalary = (totalSalary / monthsSelected).toFixed(0); // Corrected: Divide by selected months, not by monthsSelected * 12
...
<script>
    function calculateMonthlyTax() {
        ...
        const avgMonthlyTax = (taxAmount / 12).toFixed(0); // Calculate average monthly tax for user
    ...
    function calculateCoapplicantMonthlyTax() {
        ...
        const avgCoapplicantMonthlyTax = (coapplicantTaxAmount / 12).toFixed(0); // Calculate average monthly tax for co-applicant
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var calculateMonthlyTax=Math.round(calculateMonthlyTax);
        var calculateCoapplicantMonthlyTax=Math.round(calculateCoapplicantMonthlyTax);
